...
using Server.Conventions;
using Server.Formatters;
using Server.Middlewares;
using Throw;
using PathHelper = Infrastructure.Utils.PathHelper;
...
try
{
    ...
    builder.Services.AddAuthorizationBuilder()
        .AddPolicy("AuthConditional", policy =>
        {
            ...
            if (authRequired)
            {
                ...
                policy.RequireAssertion(context => uint.TryParse(context.User.FindFirst(ClaimTypes.Name)?.Value, out _));
            }
            else
            {
        ...
        .AddPolicy("AuthConditionalAdmin", policy =>
        {
            var authRequired = builder.Configuration.GetSection(nameof(AuthSettings)).GetValue<bool>("AuthenticationRequired");
            if (authRequired)
            {
